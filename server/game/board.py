...
class Board(object):
    def __init__(self):
        ...
        self.fields = self.generateBoard()
        self.points = 0
        self.diff = 100
        self.setCastle()
    ...
    def setCastle(self):
        ...
        self.fields[self.mid][self.mid] = CASTLE

    def generateBoard(self):
        ''' 
        :return [[None,None],[None,None]...]: generate board as two a two-dimensional array
        '''
        board = []
        for i in range(2*self.maxPos+1):
            board.append([])
            for j in range(2*self.maxPos+1):
                board[i].append(None)
        return board
    ...
    def checkIsCorrectMove(self, x, y, orientation):
        ...
        :param int orienation: orientation of puzzle
        ...
        if self.isCorrectPosition(x, y, orientation) and self.isCorrectPosition(x1, y1):
            ...
            if self.isEmptyField(x, y) and self.isEmptyField(x1, y1) and (self.isNearOfThePuzzle(x, y) or self.isNearOfThePuzzle(x1, y1)):
                flag = True
    ...
    def isCorrectPosition(self, x, y, orientation=0):
        ...
        if (x <= self.maxPos and x >= self.minPos) and (y <= self.maxPos and y >= self.minPos) and orientation in allowedOrientation:
            return True
        return False
...
###########################################################################
    ...
    def getValueAndBonus(self, field):
        ...
        if field is None:
            ...
            value = field[0]
            bonus = int(field[1])
        elif len(field) == len(CASTLE) and field == CASTLE:
            return None, None
        else:
    ...
    def getBonus(self, bonus):
        '''
        :param int or None: value of bonus
