...
class BoardHandlerMixin:
    def generate_board(self, max_position):
        ...
        :return [[None,None],[None,None]...]: generate board
                                              as two-dimensional array
        """
        return [
            [None] * (2 * max_position + 1)
            for _ in range(2 * max_position + 1)
        ]
...
class Board(BoardHandlerMixin):
    ...
    def check_is_correct_move(self, x, y, orientation):
        ...
        :return boolean: return true if x,y are correct value on the board,
                two fields [x,y] and [x1,y1](it depends on the orientation)
                are empty and one of these field is adjacent of
                the puzzles on the board
        """
        # check position of player
        x1, y1 = self.get_second_position_of_puzzle(x, y, orientation)
        if self.is_correct_position(
            x, y, orientation
        ) and self.is_correct_position(x1, y1):
            x, y, x1, y1 = (
                x + self.diff,
                y + self.diff,
                x1 + self.diff,
            ...
            if (
                self.is_empty_field(x, y)
                and self.is_empty_field(x1, y1)
                and (
                    self.is_near_of_the_puzzle(x, y)
                    or self.is_near_of_the_puzzle(x1, y1)
                )
            ):
                return True
    ...
    def is_empty_field(self, x, y):
        ...
        :param int x: x coordinate of puzzle,
        :param int y: y coordinate of puzzle,
        :return boolean: return false if field is equal None,
    ...
    def is_correct_position(self, x, y, orientation=0):
        ...
        return (
            (self.maxPos >= x >= self.minPos)
            and (self.maxPos >= y >= self.minPos)
    ...
    def get_second_position_of_puzzle(self, x, y, orientation=0):
        ...
        :param int y: y coordinate of puzzle,
        :param int orientation: orientation of puzzle
        :return int, int: the coordinates of second position field,
    ...
    def add_puzzle_to_the_board(self, puzzle, x, y, orientation):
        ...
        self.fields[x][y] = puzzles[0]
        x1, y1 = self.get_second_position_of_puzzle(x, y, orientation)
        self.fields[x1][y1] = puzzles[1]
        # self.print_board()
    ...
    def calculate_result(self):
        ...
        for i in range(len(self.fields)):
            for j in range(len(self.fields[i])):
                ...
                bonus = self.get_bonus(bonus)
                if field:
                    res, b = self.search_field(field, i, j)
                    self.points = self.points + (res + 1) * (bonus + 1 + b)
        return self.points

    def search_field(self, field, pos_x, pos_y):
        """
        ...
        :return int, int - amount of total adjacent specific type fields,
                           total bonus
        """
        self.fields[pos_x][pos_y] = None
        total_bonus = 0
        total_adjacent_fields = 0
        directions = [(-1, 0), (1, 0), (0, -1), (0, 1)]

        for x, y in directions:
            adjacent_fields, bonus = self.search_field_recursive(
                field, pos_x + x, pos_y + y
            )
            total_adjacent_fields += adjacent_fields
            total_bonus += bonus

        return total_adjacent_fields, total_bonus
    ...
    def search_field_recursive(self, field, i, j):
        ...
        :param int i: i coordinate of puzzle,
        :param int j: j coordinate of puzzle,
        :return int, int: amount of current adjacent specific type fields,
                          current bonus
        """
        value, bonus = self.get_value_and_bonus(self.fields[i][j])
        if value == field:
            next_value, next_bonus = self.search_field(field, i, j)
            return next_value + 1, self.get_bonus(bonus) + self.get_bonus(
                next_bonus
            )
        return 0, 0
    ...
    def get_value_and_bonus(self, field):
        ...
        """
        if field is None or field == config.CASTLE:
            return None, None
        elif len(field) == 1:
            return field[0], None
        elif len(field) == 2:
